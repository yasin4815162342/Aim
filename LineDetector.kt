...
object LineDetector {
    ...
    fun detect(pixels: IntArray, size: Int): DetectionResult {
        ...
        for (row in 0 until size) for (col in 0 until size) {
            if (lineMask[row * size + col]) { xs.add(col); ys.add(row) }
        }
        ...
        var angle = fitAngle(xs, ys)
        repeat(2) {
            val (tx, ty) = trimOutliers(xs, ys, angle)
            if (tx.size >= Tunables.minLinePixels) {
                xs = tx; ys = ty
                angle = fitAngle(xs, ys)
            }
        }
    ...
    private fun fitAngle(xs: List<Int>, ys: List<Int>): Double {
     ...
     *  caller refits — a cheap stand-in for a proper Huber/IRLS robust fit. */
    private fun trimOutliers(xs: List<Int>, ys: List<Int>, angle: Double): Pair<ArrayList<Int>, ArrayList<Int>> {
        val mx = xs.average(); val my = ys.average()
        ...
        val outXs = ArrayList<Int>(); val outYs = ArrayList<Int>()
        for (i in xs.indices) {
            if (stdR < 1e-6 || residuals[i] <= threshold) {
                outXs.add(xs[i]); outYs.add(ys[i])
            }
        }
        return Pair(outXs, outYs)
    }
